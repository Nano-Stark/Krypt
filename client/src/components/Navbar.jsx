    ...
    <nav className="w-full flex md:justify-center justify-between items-center p-4">
        ...
        <ul className="text-white md:flex hidden list-none flex-row justify-between items-center flex-initial">
              {["Market", "Exchange", "Tutorials", "Wallets"].map((item, id) => (
                  <NavbarItem key={item + id} title={item} />
              ))}
        ...
        <div className="flex relative">
          ...
          : 
            <>
              <p className="bg-[#2952e3] py-2 px-7 mx-7 text-white md:hidden rounded-full cursor-pointer hover:bg-[#2546bd]">
                    Login
              </p>
          ...
          {toggleMenu && (
            <ul 
                className="z-10 fixed top-0 right-0 p-3 w-[70vw]
              ...
              {["Market", "Exchange", "Tutorials", "Wallets"].map((item, id) => (
                  <NavbarItem key={item + id} title={item} classProps="my-2 text-lg" />
              ))}
            </ul>
          )}
